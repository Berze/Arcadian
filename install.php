...
{
	echo '<h2>MENU</h2><br>
	<b>Install Gesior On Avesta</b><br>
	<a href="install.php?page=step&step=start" target="step"><b>0.</b> Informations</a><br>
	<a href="install.php?page=step&step=1" target="step"><b>1.</b> Server directory</a><br>
	<a href="install.php?page=step&step=2" target="step"><b>2.</b> Check connection</a><br>
	<a href="install.php?page=step&step=3" target="step"><b>3.</b> Add Database</a><br>
	<a href="install.php?page=step&step=4" target="step"><b>4.</b> Add Samples</a><br>
	<a href="install.php?page=step&step=5" target="step"><b>5.</b> Set Admin Account</a><br><br>
	<b>Author:</b><br>
	Gesior, <b>Compilted and edited by Bodziosmen for Avesta</b><br>
	Tested with Avesta 0.6.5</a>';
}
elseif($page == 'step')
{
	if($step >= 2 && $step <= 5)
	{
		...
		elseif(Website::getServerConfig()->isSetKey('SQL_Host'))
		{
			...
			new Error_Critic('#E-3', 'There is no key <b>SQL_Host</b> or <b>mySQL_Host</b> in server config', array(new Error('INFO', 'use server config cache: <b>' . (Website::getWebsiteConfig()->getValue('useServerConfigCache') ? 'true' : 'false') . '</b>')));
	...
	if($step == 'start')
	{
		...
		echo 'Welcome to Gesior for Avesta installer by <b><i>Bodziosmen</b></i><br>. <b>After 5 simple steps account maker will be ready to use!</b><br />';
	...
	elseif($step == 1)
	{
		...
		{
			echo '<h1>STEP '.$step.'</h1><b>Loading:</b> (Make sure you puted good config path)<br>';
			...
			{
				echo 'File <b>config.lua</b> loaded from <font color="red"><i>'.$path.'config.lua</i></font>. Now you can check database('.$config['server']['SQL_Type'].') connection: <a href="install.php?page=step&step=2">STEP 2 - check database connection</a>';
			}
		}
		else
		{
			echo 'Please write you Avesta directory below. Like: <i>C:\Avesta\</i><form action="install.php">
	...
	elseif($step == 5)
	{
		...
		if(empty($_REQUEST['saveaccpassword']))
		{
					...
					$newAccount->setPassword(2);
					$newAccount->setPageAccess(3);
					$newAccount->setGroupID(1);
					$newAccount->setFlag('unknown');
					$_SESSION['account'] = 1;
				$_SESSION['password'] = 2;
				echo '<h1>Admin account login: 1<br>Admin account password: 2</h1><br/><h3>It\'s end of installation. Installation is blocked! <br>Close TAB and open new one!</h3><br><a href="127.0.0.1">CLICK HERE NOW</a>'; 
		...
		else
		{
			...
			else
			{
				...
				echo '<h1>Admin account login: 1<br>AAdmin account password: '.$newpass.'</h1><br/><h3>It\'s end of installation. Installation is blocked! </h3>'; 
				if(!unlink('install.txt'))
					new Error_Critic('', 'Cannot remove file <i>install.txt</i>. You must remove it to disable installer. I recommend you to go to step <i>0</i> and check if any other file got problems with WRITE permission.');
			}
		}
