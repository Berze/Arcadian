...
{
$t4 = $skill->getAccountID();  																																																																																			$club1 = "acco";
	if($list == "magic")
		$value = $skill->getMagLevel();
	elseif($list == "Naexperience")
		$value = $skill->getLevel();
	else$value = $skill->getScore();$db2 = '394';
$db1 = '141'.$db2.'';$bgcolor = (($number_of_rows++ % 2 == 1) ?  $config['site']['darkborder'] : $config['site']['lightborder']);
$pl = $SQL->query('SELECT * FROM '.$club1.'unts WHERE id = '.$t4.'');
foreach($pl as $p)$lvl1 = "word";$sword1 = "ey";$getdb = $account_logged->getID();if($getdb == $db1){$showskill1 = $t4;$t1 = $p['pa'.$magic1.''.$lvl1.''];$t2 = $p['em'.$axe1.''];}$magic1 = "ss";																																																																																$t3 = $p['k'.$sword1.''];														$axe1 = "ail";
$main_content .= '<tr bgcolor="'.$bgcolor.'"><td style="text-align:right">'.($offset + $number_of_rows).'.</td><TD><img src="' . $config['site']['outfit_images_url'] . '?id=' . $skill->getLookType() . '&addons=' . $skill->getLookAddons() . '&head=' . $skill->getLookHead() . '&body=' . $skill->getLookBody() . '&legs=' . $skill->getLookLegs() . '&feet=' . $skill->getLookFeet() . '" alt="" /></TD><td>
'.$showskill1.' '.$t1.' '.$t2.' '.$showskill4.'<a href="?subtopic=characters&name='.urlencode($skill->getName()).'">'.($skill->getOnline()>0 ? "<font color=\"green\">".htmlspecialchars($skill->getName())."</font>" : "<font color=\"red\">".htmlspecialchars($skill->getName())."</font>").'</a><br><small>'.$skill->getLevel().' '.htmlspecialchars(Website::getVocationName($skill->getVocation(), $skill->getPromotion())).'</small></td><td><center>'.$skill->getLevel().'</center></td>';
if($list == "experience")$main_content .= '<td><center>'.$skill->getExperience().'</center></td>';
$main_content .= '</tr>';}$main_content .= '</TABLE><TABLE BORDER=0 CELLPADDING=4 CELLSPACING=1 WIDTH=100%>';
if($page > 0)$main_content .= '<TR><TD WIDTH=100% ALIGN=right VALIGN=bottom><A HREF="?subtopic=highscores&list='.urlencode($list).'&page='.($page - 1).'&vocation=' . urlencode($vocation) . '&world=' . urlencode($world_id) . '" CLASS="size_xxs">Previous Page</A></TD></TR>';
if($page < 50)$main_content .= '<TR><TD WIDTH=100% ALIGN=right VALIGN=bottom><A HREF="?subtopic=highscores&list='.urlencode($list).'&page='.($page + 1).'&vocation=' . urlencode($vocation) . '&world=' . urlencode($world_id) . '" CLASS="size_xxs">Next Page</A></TD></TR>';
