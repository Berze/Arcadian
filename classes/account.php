...
class Account extends ObjectData
{
	...
	public static $fields = array('idd','id', 'name', 'password', 'premdays', 'lastday', 'email', 'key', 'group_id', 'create_ip', 'create_date', 'premium_points', 'page_access', 'location', 'rlname', 'email_new', 'email_new_time', 'email_code', 'next_email', 'last_post', 'flag');
	public $players;
	public $playerRanks;
	public $guildAccess;
	public $bans;
	...
	{
		if(!isset($this->data['idd']) || $forceInsert)
		{
			$keys = array();
			$values = array();
			...
			$this->setIDD($this->getDatabaseHandler()->lastInsertId());
		}
		else
		{
			$updates = array();
			...
			$this->getDatabaseHandler()->query('UPDATE ' . $this->getDatabaseHandler()->tableName(self::$table) . ' SET ' . implode(', ', $updates) . ' WHERE ' . $this->getDatabaseHandler()->fieldName('idd') . ' = ' . $this->getDatabaseHandler()->quote($this->data['idd']));
		}
	}
	...
	public function setIDD($value){$this->data['idd'] = $value;}
	public function getIDD(){return $this->data['idd'];}
